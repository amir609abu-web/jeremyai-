      ...
      <div className="mt-4 divide-y divide-border-glass">
        {EVENTS.map((e, i) => (
          <div key={i} className="flex items-center gap-4 py-3 text-sm">
            <div className="w-14 shrink-0 text-xs text-muted-2">
              <div>{t(e.day)}</div>
              <div dir="ltr">{e.time}</div>
            </div>
            <span
              className="w-10 shrink-0 rounded border border-border-glass px-1.5 py-0.5 text-center text-[10px] font-semibold text-muted"
              dir="ltr"
            >
              {e.currency}
            </span>
